...
async def afficher_formulaire_client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Prénom et Nom
    row1 = []
    row1.append(InlineKeyboardButton(f"👤 Prénom: {prenom or '?'}", callback_data='modif_prenom'))
    row1.append(InlineKeyboardButton(f"👤 Nom: {nom or '?'}", callback_data='modif_nom'))
    keyboard.append(row1)
    
    # Téléphone et Email
    row2 = []
    row2.append(InlineKeyboardButton(f"📞 Tél: {client.get('telephone') or '?'}", callback_data='modif_telephone'))
    row2.append(InlineKeyboardButton(f"📧 Email: {client.get('email') or '?'}", callback_data='modif_email'))
...
async def modif_champ(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if champ == 'methode':
        ...
        await query.edit_message_text(messages[champ], reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    elif champ == 'voyages':
        voyages = db.get_tous_voyages()
        keyboard = []
...
async def valider_client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🔙 RETOUR MENU", callback_data='retour_menu')]]
    await query.message.reply_text("Retour au menu principal ?", reply_markup=InlineKeyboardMarkup(keyboard))
    context.user_data.clear()
...
async def menu_voyages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if voyages:
        ...
        for voyage in voyages:
            ...
            keyboard.append([InlineKeyboardButton(f"{couleur} {nom} ({date_voyage or '?'})", callback_data=f'voyage_detail_{voyage_id}')])
...
async def voyage_recevoir_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    couleurs = ["🔴", "🟠", "🟡", "🟢", "🔵", "🟣", "🟤", "⚫", "⚪"]
    keyboard = []
    row1 = [InlineKeyboardButton(c, callback_data=f'voyage_couleur_{c}') for c in couleurs[:3]]
    row2 = [InlineKeyboardButton(c, callback_data=f'voyage_couleur_{c}') for c in couleurs[3:6]]
    row3 = [InlineKeyboardButton(c, callback_data=f'voyage_couleur_{c}') for c in couleurs[6:9]]
    keyboard.append(row1)
    keyboard.append(row2)
...
async def voyage_choisir_couleur(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    voyage_id = db.ajouter_voyage(voyage_data['nom'], voyage_data.get('date', ''), couleur)
    await query.edit_message_text(
        f"✅ *VOYAGE CRÉÉ !*\n\n"
    ...
    await query.message.reply_text("Que voulez-vous faire ?", reply_markup=InlineKeyboardMarkup(keyboard))
    context.user_data.clear()
...
async def voyage_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if clients:
        ...
        for client in clients[:10]:
            ...
            keyboard.append([InlineKeyboardButton(f"👤 {prenom} {nom}", callback_data=f'detail_{client_id}')])
    else:
...
# ---------- AUTRES FONCTIONNALITÉS (simplifiées pour l'instant) ----------
async def rechercher_client(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("🔍 Fonction de recherche à venir...")

async def liste_clients(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("📋 Liste des clients à venir...")

async def prochains_paiements(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("💰 Prochains paiements à venir...")

async def statistiques(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("📊 Statistiques à venir...")

async def clients_termines(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("📁 Clients terminés à venir...")

async def export(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("📤 Export à venir...")

async def paiement_recu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("💰 Paiement reçu à venir...")
