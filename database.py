...
class Database:
    ...
    def ajouter_client(self, nom, telephone="", email="", description="", montant_du=0, date_limite=""):
        self.c.execute('''INSERT INTO clients
                         ...
                         VALUES (%s, %s, %s, %s, %s, %s, %s, 'actif') RETURNING id''',
    ...
    def rechercher_client(self, recherche):
        ...
        self.c.execute('''SELECT * FROM clients
                         WHERE nom ILIKE %s AND statut = 'actif'
                         ORDER BY
    ...
    def modifier_client(self, client_id, champ, valeur):
        champs_autorises = ['nom', 'telephone', 'email', 'description', 'montant_du', 'date_limite']
    ...
    def get_paiements_imminents(self, jours=30):
        ...
        for client in tous_clients:
            client_id, nom, _, _, _, montant_du, date_limite, _, _ = client
            ...
            try:
                ...
                continue
        clients_imminents.sort(key=lambda x: x[6] if x[6] else '9999-12-31')
